...
def convert_dataset():
    ...
    IMG_DIR = '/home/aistudio/data/dehw_train_dataset/images'
    BG_IMG_DIR = '/home/aistudio/data/dehw_train_dataset/gts'
    BBOX_DIR = '/home/aistudio/dataset/bbox'
    TEST_DIR ='/home/aistudio/data/dehw_testA_dataset/images'
    DATASET_DIR = './output'
    if not os.path.exists(DATASET_DIR):
        os.mkdir(DATASET_DIR)
    
    all_files = glob.glob(IMG_DIR + '/*.jpg')
    print("Total %d train files are found" % (len(all_files)))
    data = []
    ...
    print("Total %d test files are found" % (len(test_files)))

    data = []
    for item in test_files:
        img_name = os.path.splitext(os.path.basename(item))[0]
        #print(img_name)
        item = TextMaskData(TEST_DIR, '', img_name, np.array([]), np.array([]))
        if item.isValid:
            data.append(item)
