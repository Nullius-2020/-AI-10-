...
input_dir =  '/home/aistudio/data/dehw_train_dataset'
output_dir = os.path.join('/home/aistudio/dataset', 'bbox')

if not os.path.exists(output_dir):
    os.mkdir(output_dir)

images = glob2.glob(os.path.join(input_dir, 'images', '*.jpg'))
print("#item: ", len(images))
DEGBUG = False

for i, item in enumerate(images):
    label_img_path = os.path.join(input_dir, 'gts', os.path.basename(item).split('.')[0]+'.png')
    img = cv2.imread(item)
