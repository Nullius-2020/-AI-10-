...
class MaskInpaintModel(BaseModel):
    def __init__(self, config):
        ...
        self.gen_scheduler =paddle.optimizer.lr.StepDecay(learning_rate=float(config.LR), step_size=config.STEP_SIZE, gamma=0.1)
        self.dis_scheduler =paddle.optimizer.lr.StepDecay(learning_rate=float(config.LR) * float(config.D2G_LR), step_size=config.STEP_SIZE, gamma=0.1)
    ...
    def process(self, images, images_gt, masks, masks_gt, masks_refine_gt, use_gt_mask=False):
        ...
        pre_gen_l1_loss = paddle.mean(self.l1_loss(pre_output_images, images_gt) * l1_loss_inner_weight) * l1_loss_weight
