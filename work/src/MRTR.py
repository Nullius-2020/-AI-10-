...
class MRTR():
    ...
    def eval_epoc(self):
        ...
        with paddle.no_grad():
            for _iteration, items in enumerate(val_loader):
                ...
                mae = (paddle.sum(paddle.abs(images_gt - pre_images_gen)) / paddle.sum(images_gt)).astype('float32')
                psnr_cmp = self.psnr(self.postprocess(images_gt), self.postprocess(pre_images_cmp))
                mae_cmp = (paddle.sum(paddle.abs(images_gt - pre_images_cmp)) / paddle.sum(images_gt)).astype('float32')
    ...
    def get_inputs(self, items):
        ...
        images, images_gt, masks, masks_refine_gt, masks_refine_gt = items
        ...
        masks_refine_gt=paddle.to_tensor(masks_refine_gt,dtype='float32')

        images = images.transpose([0,3,1,2])
        images_gt = images_gt.transpose([0,3,1,2])
        masks = masks.unsqueeze(-1).transpose([0,3,1,2])
        masks_refine_gt = masks_refine_gt.unsqueeze(-1).transpose([0,3,1,2])
